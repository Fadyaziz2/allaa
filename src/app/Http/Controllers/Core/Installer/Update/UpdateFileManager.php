...
class UpdateFileManager
{
    ...
    public function removeCachedFile(): static
    {
    ...
    public function extract($version)
    {
        ...
        foreach ($all_downloaded_files as $file_path) {
            ...
            } elseif (strpos($file_path, $version . '.sql')) {

            } elseif (!strpos($file_path, 'theme29.php')) {

                if (in_array('public', $exploded) && $file_path->getFilename() != 'build/manifest.json') {
        ...
        //used to move manifest.json file
        $file_path = public_path($path . $this->separator . $version . $this->separator . 'public' . $this->separator . 'build' . $this->separator . 'manifest.json');
        if ($this->file->exists($file_path)) {
            $this->file->move($file_path, base_path('public' . $this->separator . 'build' . $this->separator . 'manifest.json'));
        }
        ...
        //used to move theme29.php file
        $file_path = public_path($path . $this->separator . $version . $this->separator . 'config' . $this->separator . 'theme29.php');
        if ($this->file->exists($file_path)) {
            $this->file->move($file_path, base_path('config' . $this->separator . 'theme29.php'));
        }
        if ($executableFile) {
