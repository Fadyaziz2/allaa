...
<div class="receipt">
    ...
    <div class="header-border-top"></div>
    <div class="details">
        <p><b>{{ __('default.invoice_pdf.invoice_no') }} #:</b> {{ $invoice->invoice_full_number }}</p>
        <p><b>{{ __('default.invoice_pdf.issue_date') }}:</b>
            {{ \Carbon\Carbon::parse()->parse($invoice->issue_date)->format($systemSettings['date_format']) }} ,
            {{ \Illuminate\Support\Carbon::parse()->parse($invoice->created_at)->timezone($systemSettings['time_zone'])->format('g:i A') }}</p>
        <p><b>{{ __('default.invoice_pdf.customer') }}:</b> {{ $invoice->customer->full_name }}</p>
    </div>
    ...
    <table class="items">
        ...
        <tr>
            ...
            <td style="text-align: right;"><b>{{ number_with_currency_symbol($invoice->sub_total) }}</b></td>
        </tr>
        <tr>
            <td></td>
            <td style="text-align: right; ">
            ...
            <td style="text-align: right;">
                <b>
                    @if ($invoice->discount_type === 'percentage')
                        {{ number_with_currency_symbol(($invoice->sub_total * $invoice->discount_amount) / 100) }}
                    @else
                        {{ number_with_currency_symbol($invoice->discount_amount) }}
                    @endif
                </b>
            </td>
        </tr>
        <tr>
            ...
            <td style="text-align: right;"><b>{{ number_with_currency_symbol($invoice->total_amount) }}</b></td>
        </tr>
        ...
        <tr>
            ...
            <td style="text-align: right;"><b>{{ number_with_currency_symbol($invoice->grand_total) }}</b></td>
        </tr>
        <tr>
            <td></td>
            <td style="text-align: right;"><b>{{ __('default.invoice_pdf.received_amount') }}</b></td>
            <td style="text-align: right;"><b>{{ number_with_currency_symbol($invoice->received_amount) }}</b></td>
        </tr>
        <tr>
            <td></td>
            <td style="text-align: right;"><b>{{ __('default.invoice_pdf.due_amount') }}</b></td>
            <td style="text-align: right;"><b>{{ number_with_currency_symbol($invoice->due_amount) }}</b></td>
        </tr>
        </tfoot>
    </table>
