        ...
        <form v-else>
            <div class="row update-details p-3 mb-4 mx-0">
                ...
                <div class="mb-3 col-md-4">
                    ...
                    <small class="text-danger" v-if="errors.recurring">{{
                            errors.recurring[0]
                        }}</small>
                </div>
                <div class="mb-3 col-md-4">
...
const submit = (type = null) => {
    ...
    if (props.id) {
        ...
        Axios.patch(`${INVOICES}/${props.id}`, data)
            .then(({data}) => {
                toast.success(data?.message);
                formData.value.tax_id = null;
                if (invoiceSetting.value.thermal_printer === 'enable'){
                    router.push(`thermal-invoice/${data.invoice.id}`);
                }else{
...
const getInvoiceEditData = () => {
    pageLoader.value = true;
    Axios.get(`${INVOICES}/${props.id}`)
        .then(({data}) => {
            formData.value = data;
            selectedInvoiceTemplate.value = data.invoice_template;
            productDetails.value = data.invoice_details.map((item) => {
                return {
                    id: item.id,
