...
class _UnitsScreenState extends State<UnitsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Get.find<ProductController>().getUnitsManagementList();
    });
  }

  Future<void> _scrollListener() async {
    if (_scrollController.position.pixels == _scrollController.position.maxScrollExtent) {
      if (!Get.find<ProductController>().unitsManagementPaginateLoading &&
          Get.find<ProductController>().unitsManagementNextPageUrl != null) {
        await Get.find<ProductController>().getUnitsManagementList(isPaginate: true);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        ...
        title: 'units_key'.tr,
        actions: [
          if (permissionData!.isAppAdmin! || permissionData.createUnits!)
            GestureDetector(
              onTap: () {
                    ...
                    builder: (productController) {
                      return ConfirmationDialog(
                        ...
                        titleBodyWidget: Column(
                          children: [
                            Text('add_unit_key'.tr, style: poppinsBold.copyWith(fontSize: Dimensions.FONT_SIZE_LARGE)),
                        ...
                        rightBtnOnTap: () {
                          if (_nameController.text.trim().isEmpty) {
                            showCustomSnackBar('please_enter_the_unit_name_key'.tr, isError: true);
                            return;
                          }
                          productController.addUnit(name: _nameController.text.trim()).then((value) {
                            if (value.isSuccess) {
                              Get.back();
      ...
      body: GetBuilder<ProductController>(
        builder: (productController) {
          if (productController.unitsManagementLoading) {
            return const Center(child: LoadingIndicator());
          }
          if (productController.unitsManagementList.isEmpty) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              if (index == productController.unitsManagementList.length) {
            ...
            itemCount: productController.unitsManagementList.length + (productController.unitsManagementPaginateLoading ? 1 : 0),
          );
        },
      ),
    );
